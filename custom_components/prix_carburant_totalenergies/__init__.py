...
async def _get_total_station_coordinates(hass, latitude, longitude, radius_km):
    ...
    # No amenity=fuel restriction: some Total sites are mapped with a
    # different OSM object type. We identify the brand/name/operator instead.
    ...
    for url in OVERPASS_URLS:
        try:
            ...
            if stations:
                _LOGGER.debug("%s stations TotalEnergies trouvées via %s", len(stations), url)
                return stations
...
def parse(raw, cfg, total_stations):
    ...
    wanted = {str(x) for x in cfg.get("stations", [])}
    lat0, lon0, radius = float(cfg["latitude"]), float(cfg["longitude"]), float(cfg["radius_km"])
    if total_stations is None and not wanted: return {}
    total_coords = [(lat, lon) for lat, lon, _ in (total_stations or [])]
    result = {}
    for pdv in root.iter():
        ...
        elif not total_coords or min(distance(lat, lon, a, b) for a, b in total_coords) > 2.0:
            continue
        fuels, ruptures = {}, set()
        ...
        result[station_id] = {"station_id": station_id, "latitude": lat, "longitude": lon,
            "distance_km": round(dist,2), "postal_code": attrs.get("cp"),
            "address": _text(pdv,"adresse"), "city": _text(pdv,"ville"), "fuels": fuels,
...
class Coordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        try:
            ...
            if not data:
                raise UpdateFailed("Aucune station TotalEnergies trouvée dans le rayon configuré")
            return data
